...
def predict(forest_file, input_image_folder, input_image_suffix, input_features_suffix, output_folder, feature_list):
    ...
    tot_err = 0
    for im_file, feature_file in zip(im_files, feature_files):
        ...
        output_image_basename = im_file.replace(input_image_folder, output_folder)
        ...
        mask_b = im[:, :, 0] > np.maximum(im[:, :, 1], im[:, :, 2])
        mask_g = im[:, :, 1] > np.maximum(im[:, :, 0], im[:, :, 2])
        mask_r = im[:, :, 2] > np.maximum(im[:, :, 0], im[:, :, 1])
